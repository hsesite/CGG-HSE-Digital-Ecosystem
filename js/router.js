...
(() => {
  ...
  const Router = {
      ...
      this.routes = {
        ...
        upload: async () => {
          if (window.HDOSRepositoryUpload?.render) {
            await window.HDOSRepositoryUpload.render();
          } else {
        ...
        inspection: async () => {
          if (window.Inspection?.render) {
            await window.Inspection.render();
          } else {
    ...
    async renderDynamic(module, title) {
      ...
      try {
        const data = await window.CGGLoader?.schema?.(module);

        if (!data || !data.success) {
          throw new Error("Schema gagal dimuat.");
        }

        if (window.CGGRenderer?.mount) {
          window.CGGRenderer.mount("#dynamic-form", data.schema);
          console.log(`✓ Dynamic module "${module}" loaded.`);
        } else {
          throw new Error("CGGRenderer belum dimuat.");
        }

      } catch (error) {
        console.error("Router renderDynamic failed:", error);
