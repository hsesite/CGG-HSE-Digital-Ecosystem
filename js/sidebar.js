...
const Sidebar = {
  ...
  async template() {
    ...
    try {
      ...
      console.warn("Passport check skipped", e);
    }
...
<div class="sb-shell">
  <div class="sb-header">
    ...
    <img src="/CGG-HSE-Digital-Ecosystem/assets/Logo/logo-cgg.png" onerror="this.src='assets/Logo/logo-cgg.png'">
    <div class="sb-brand">
      <b>CGG HDOS</b>
...
`;
    ...
    CATEGORY_ORDER.forEach(cat => {
      ...
      html += `
    <div class="sb-group">
      <div class="sb-title">${CATEGORY_TITLE[cat] || cat}</div>
`;

      list.forEach(m => {
        const iconKey = ICON_MAP[m.module] || m.icon || "grid";
        const iconSvg = SVG[iconKey] || SVG.grid;
        const titleText = m.title || TITLE_MAP[m.module] || m.module;

        html += `
      <button class="sb-item" data-route="${(m.route || ("#" + m.module)).replace("#", "")}">
        ${iconSvg}
        <span>${titleText}</span>
      </button>
`;
