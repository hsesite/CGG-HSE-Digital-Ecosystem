...
(() => {
  ...
  const SVG = {
    ...
    settings: `<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19 12a7 7 0 0 0-.1-1l2-1-2-3-2 1a7 7 0 0 0-2-1l-.3-2H10l-.3 2a7 7 0 0 0-2 1l-2-1-2 3 2 1a7 7 0 0 0 0 2l-2 1 2 3 2-1a7 7 0 0 0-2-1l-.3-2H10l-.3 2a7 7 0 0 0-2 1l-2-1-2 3 2 1a7 7 0 0 0 0 2l-2 1 2 3 2-1a7 7 0 0 0-2-1l-.3-2H10l-.3 2a7 7 0 0 0-2 1l-2-1-2 3 2 1a7 7 0 0 0 0 2l-2 1 2 3 2-1a7 7 0 0 0-2-1l-.3-2H10l-.3 2a7 7 0 0 0-2 1l-2-1-2 3 2 1v0"/></svg>`,
  ...
  const STATIC_MODULES = [
    ...
    { module: "policy", title: "Policy", icon: "settings" },
  ...
  const Sidebar = {
    ...
    bind() {
      document.querySelectorAll(".sb-item").forEach(button => {
        button.onclick = () => {
          if (window.Router) {
            window.Router.navigate(button.dataset.route);
          }

          this.activate(button.dataset.route);
    ...
    async loadRegistry() {
      ...
      try {
        ...
        console.warn("Sidebar fallback cache aktif.");
        this.cache = [];
      }
    ...
    async template() {
      ...
      registry.forEach(item => {
        const index = merged.findIndex(m => m.module === item.module);
      ...
      if (window.CGGRole?.current) {
        passport = await window.CGGRole.current();
      }
      ...
      for (const module of merged) {
        ...
        if (
          passport.scope.includes(module.company) ||
      ...
      CATEGORY_ORDER.forEach(category => {
        ...
        html += `
          <div class="sb-group">
            <div class="sb-title">${CATEGORY_TITLE[category]}</div>
          `;

        list.forEach(module => {
          const iconName = ICON_MAP[module.module] || module.icon || "grid";

          html += `
            <button class="sb-item" data-route="${String(module.route || module.module).replace("#", "")}">
              ${SVG[iconName] || SVG.grid}
              <span>${module.title || TITLE_MAP[module.module] || module.module}</span>
            </button>
          `;
        });
